 ...
 *
 * Created by Sally Goldin, 9 August Modified to use IOUtils.java - 19 Jan 2020
 * 
...
public class SquareTesterGraphics {
    /**
     * Keep hold of the last square we created so we can move it using the mouse.
     */
	...
	while (bContinue)
		...
		latestSquare.draw(viewer.getGraphics2D(), false);
		...
		if (move.startsWith("Y"))
		    ...
		    latestSquare.draw(viewer.getGraphics2D(), true);
		    }
		System.out.println("-----------------------------\n");
		}
	    }	
	Square.drawAllSquare(viewer.getGraphics2D());
    	}
