/**
 * Class that reads from a text file defining shapes and creates instances of
 * AbstractShape subclasses. Possible lines in the file include: TRIANGLE x1 y1
 * x2 y2 x3 y3 SQUARE x y side DIAMOND x y vaxis haxis CIRCLE x y radius
 * 
...
public class ShapeReader extends TextFileReader
{
	...
	/**
	 * Determine if the fields are correct for a triangle, and if so, create one.
	 * 
	 * @param fields Array of strings parsed from file line. We have already checked
	 *               the first field is 'TRIANGLE'
	...
	/**
	 * Determine if the fields are correct for a square, and if so, create one.
	 * 
	 * @param fields Array of strings parsed from file line. We have already checked
	 *               the first field is 'SQUARE'
	...
	/**
	 * Determine if the fields are correct for a diamond, and if so, create one.
	 * 
	 * @param fields Array of strings parsed from file line. We have already checked
	 *               the first field is 'DIAMOND'
	...
	/**
	 * Determine if the fields are correct for a circle, and if so, create one.
	 * 
	 * @param fields Array of strings parsed from file line. We have already checked
	 *               the first field is 'CIRCLE'
	...
	/**
	 * check line from shape file to make sure it has correct shape type and correct
	 * arguments.
	 * 
	 * @param line String read from the file
	 * @return The shape specified by the command line, or null if the line contains
	 *         any errors.
	 */
	...
	/**
	 * Read the next line from the file. Parse it and return the appropriate type of
	 * shape based on the content of the line. If an error is found, just skips to
	 * the next line, until it gets to a good line or the end of the file.
	 * 
	 * @return specific subclass instance of AbstractShape, or null if the end of
	 *         the file.
	 */
